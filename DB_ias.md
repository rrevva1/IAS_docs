...
|------------|---------------------------|
| Создание и восстановление БД ias_vniic | macOS/Linux: `scripts/create_and_restore_ias_vniic.sh` (дамп `db/IAS_VNIIC_dump.sql`). Windows: создать БД `ias_vniic` и выполнить дамп через psql (при необходимости добавить `create_and_restore_ias_vniic.cmd`). Имя БД везде в нижнем регистре — один конфиг на всех ОС. |
...
## Работа с БД в команде (macOS / Windows)

**Идея:** схема и эталонный дамп хранятся в репозитории; у каждого разработчика — своя локальная копия БД с тем же именем, восстановленная из дампа.

| Шаг | Кто | Действие |
|-----|-----|----------|
| 1 | Коллега (владелец схемы) | Делает дамп: `pg_dump -h localhost -U postgres -d ias_vniic --no-owner --no-acl -f db/IAS_VNIIC_dump.sql` и коммитит/пушит файл. |
| 2 | Вы (и любой в команде) | После `git pull` восстанавливаете БД локально из `db/IAS_VNIIC_dump.sql`. |
| 3 | Работа | Каждый работает со своей локальной БД **ias_vniic** (имя в нижнем регистре — одинаково на Mac и Windows); при изменении схемы/данных снова делается дамп, файл обновляется в репо, остальные подтягивают и пересоздают/восстанавливают локально при необходимости. |

**Восстановление БД из дампа:**

- **macOS / Linux:**  
  `PGPASSWORD=12345 ./scripts/create_and_restore_ias_vniic.sh`  
  (скрипт создаёт БД **ias_vniic** и выполняет `db/IAS_VNIIC_dump.sql`.)

- **Windows:**  
  Аналогичный порядок в cmd/PowerShell: создать БД **ias_vniic** (без кавычек, lowercase), затем  
  `psql -h localhost -U postgres -d ias_vniic -f db\IAS_VNIIC_dump.sql`.  
  При необходимости можно добавить `scripts/create_and_restore_ias_vniic.cmd` для Windows (создание БД + вызов psql с дампом).

**Регистр имени БД:** везде используется **ias_vniic** (нижний регистр), чтобы в `config/db.php` было одинаково на всех машинах (`dbname=ias_vniic`), без смены под ОС.

Подключение приложения: в конфиге указать `dbname=ias_vniic`, хост/пользователь/пароль — по окружению каждой машины.