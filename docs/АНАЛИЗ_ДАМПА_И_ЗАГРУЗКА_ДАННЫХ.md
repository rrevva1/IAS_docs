...
| **Текущая (baseline)** | **ias_vnii_db** — `db/ias_vnii_db.sql` | Развитая схема для проектируемой ИАС: единый стиль `id`/`*_id`, таблицы `equipment`/`equip_history`, доп. таблицы (ПО, лицензии, аудит, следы Django). |

ИАС будет работать с данными из Основного учета; целевая схема для хранения — **ias_vnii_db** (после удаления из неё объектов, не относящихся к доменной БД — см. раздел про Django).
...
В **ias_vnii_db** дополнительно есть: **software**, **licenses**, **software_installs** (учёт ПО и лицензий), **audit_events** (аудит). Таблицы Django/auth к доменной БД не относятся и подключаются при интеграции с приложением позже.
...
| **auth_group**, **auth_permission**, **auth_group_permissions** | Группы и права (Django Auth) |
| **django_admin_log**, **django_content_type**, **django_migrations**, **django_session** | Служебные таблицы Django |
...
- В репозитории **нет** готового кода загрузки из этого файла в БД ни в `ias_php`, ни в `ias_py`. В `ias_py/docs/technical/ASSETS_DOMAIN.md` указано: «реализовать импорт активов (CSV/XLSX)» как последующий шаг. В `ias_py/docs/guides/pr_perenos.md` описан перенос данных из старой БД (Yii2 → Django), а не из Excel.
...
3. **Реализовать загрузчик** (предпочтительно Python: pandas + openpyxl + psycopg2 или SQLAlchemy), который:
...
| Загрузка из «Основного учета» | Готового кода нет. Нужен скрипт (Python + Excel + PostgreSQL): маппинг столбцов Excel → таблицы БД, порядок вставки с учётом FK. Шаблон — `scripts/load_osnovnoy_uchot.py`. |
