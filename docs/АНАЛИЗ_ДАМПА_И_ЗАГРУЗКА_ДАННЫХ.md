...
Восстановление БД IAS_VNIIC: macOS/Linux — `scripts/create_and_restore_ias_vniic.sh`; Windows — создать БД и выполнить дамп через psql (раздел «Работа с БД в команде» в `DB_ias.md`).
...
| Создание БД ias_vniic | macOS/Linux: `scripts/create_and_restore_ias_vniic.sh`. Windows: создать БД `ias_vniic`, выполнить `psql -d ias_vniic -f db\IAS_VNIIC_dump.sql`. Имя в нижнем регистре — один конфиг на всех ОС. |
