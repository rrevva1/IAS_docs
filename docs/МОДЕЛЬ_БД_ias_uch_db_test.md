# Модель БД `ias_uch_db_test`

## 1. Назначение

Документ определяет логическую и физическую модель базы данных `ias_uch_db_test`, разработанной на основе текущей схемы с расширением до полного покрытия ключевых требований ТЗ, включая:
...
DDL-реализация размещена в файле `scripts/create_ias_uch_db_test.sql`.
...
- СУБД: PostgreSQL.
- Имя БД: `ias_uch_db_test`.
- Кодировка: `UTF8`.
- Схема: `public` (базовый вариант без выделения отдельных схем).
...
Схема `ias_uch_db_test` является расширением текущей модели и покрывает критичные разрывы исходной схемы по вложениям, активам, связям заявок с активами, поиску/фильтрации и аудиту. В результате модель пригодна для реализации полного объема приемки по указанным разделам `TZ.md`.
